...
%% Code
%
function [output_string, var_out] = write_SaturationDynamic(block, ir_struct, varargin)

var_out = {};

outMin = LusUtils.getParamValue(ir_struct, block, block.OutMin);
outMax = LusUtils.getParamValue(ir_struct, block, block.OutMax);
...
if numel(outMin) ~= 0
    [list_cstMin] = Utils.list_cst(outMin, block.CompiledPortDataTypes.Inport(2));
    if numel(list_cstMin) == 1 && block.CompiledPortWidths.Outport ~= 1
...
if numel(outMax) ~= 0
    [list_cstMax] = Utils.list_cst(outMax, block.CompiledPortDataTypes.Inport(2));
    if numel(list_cstMax) == 1 && block.CompiledPortWidths.Outport ~= 1
