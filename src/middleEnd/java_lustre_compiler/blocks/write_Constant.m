...
%% Code
%
function [output_string, var_out] = write_Constant(unbloc, ir_struct, varargin)

Kvalue = LusUtils.getParamValue(ir_struct, unbloc, unbloc.Value);
output_string = '';
var_str = '';
...
if is_bus
	...
	if unbloc.CompiledPortComplexSignals.Outport(1)
		% Complex constant
		dt = LusUtils.get_lustre_dt(unbloc.CompiledPortDataTypes.Outport(1));
		for idx_row=1:dim_r
			for idx_col=1:dim_c
...
blk_type = cocoget_param(ir_struct, unbloc.Post(1), 'BlockType');
if strcmp(blk_type,'Merge')
    annotation = regexprep(num2str(unbloc.Post(1)),'\.','_');
    name = strcat('Merge_',annotation,'_input',num2str(unbloc.PortConnectivity.DstPort(1)),'_hasChanged');
