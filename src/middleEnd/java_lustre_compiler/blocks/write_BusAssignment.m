...
%% Code
%
function [output_string, var_out] = write_BusAssignment(block, ir_struct, varargin)

var_out = {};

assigned = block.AssignedSignals;
...
if is_virtual
    ...
    for idx=1:numel(block.InputSignals)
        ...
        if block.CompiledPortDimensions.Inport(cpt_bus_dims) == 1
            ...
            data{idx}.Dimensions = block.CompiledPortDimensions.Inport(cpt_bus_dims+1);
            cpt_bus_dims = cpt_bus_dims + 2;
        else
            cpt_bus_elements = cpt_bus_elements + (block.inports_dim(cpt_bus_dims+1) * block.CompiledPortDimensions.Inport(cpt_bus_dims+2));
            data{idx}.Dimensions = [block.CompiledPortDimensions.Inport(cpt_bus_dims+1) block.CompiledPortDimensions.Inport(cpt_bus_dims+2)];
    ...
    for idx=2:block.Ports(1)
        index_data = find(cellfun(@(x) strcmp(x, assigned{idx-1}), block.InputSignals));
        data{index_data}.start_idx = index_first_assignment;
        index_first_assignment = index_first_assignment + block.CompiledPortWidths.Inport(idx);
    end
else
