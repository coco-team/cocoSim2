...
%% Code
%
function [output_string, var_out] = write_DiscreteZeroPole(block, ir_struct, varargin)

var_out = {};

zero = LusUtils.getParamValue(ir_struct, block, block.Zeros);
poles = LusUtils.getParamValue(ir_struct, block, block.Poles);
gain = LusUtils.getParamValue(ir_struct, block, block.Gain);
...
[list_zeros] = Utils.list_cst(zero, block.CompiledPortDataTypes.Inport(1));
[list_poles] = Utils.list_cst(poles, block.CompiledPortDataTypes.Inport(1));
[list_gain] = Utils.list_cst(gain, block.CompiledPortDataTypes.Inport(1));
