...
%% Code
%
function [output_string, var_out] = write_Bias(block, ir_struct, varargin)

var_out = {};

bias = LusUtils.getParamValue(ir_struct, block, block.Bias);

output_string = '';

[list_out] = list_var_sortie(block);
[list_in] = list_var_entree(block, ir_struct);
[list_const] = Utils.list_cst(bias, block.CompiledPortDataTypes.Outport(1));
