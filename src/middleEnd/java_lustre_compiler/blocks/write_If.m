...
%% Code
%
function [output_string, var_out] = write_If(block, ir_struct, varargin)

var_out = {};

if_expr = block.IfExpression;
elseif_expr = block.ElseIfExpressions;
num_var = LusUtils.getParamValue(ir_struct, block, block.NumInputs);
show_else = block.ShowElse;
...
for idx_in=1:block.Ports(1)
    [in_r in_c] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Inport, idx_in);
    if in_r == 1 && in_c == 1
...
if_expr = convert_to_double(if_expr, block);

% Replace 0 with 0.0 if necessary
for idx_else=1:numel(elseif_expr)
    elseif_expr{idx_else} = convert_operators(elseif_expr{idx_else});
    elseif_expr{idx_else} = convert_to_double(elseif_expr{idx_else}, block);
end
...
convert = false;
double_in_idx = find(cellfun(@(x) strcmp(LusUtils.get_lustre_dt(x), 'real'), unbloc.CompiledPortDataTypes.Inport));
