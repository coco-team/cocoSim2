...
%% Code
%
function [output_string, var_out] = write_Selector(block, ir_struct, varargin)
...
output_sizes = block.OutputSizes;
...
[dims_in{1} dims_in{2}] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Inport, 1);
...
[dims_out{1} dims_out{2}] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Outport, 1);
    ...
    for idx_out_r=1:dims_out{1}
        ...
        for idx_out_c=1:dims_out{2}
            ...
            elseif ~rows_as_input & cols_as_input
                ...
                output_string = app_sprintf(output_string, '\t%s = %s%s;\n', list_out{idx_var_out}, if_cond, list_in{idx_last});
            elseif rows_as_input & cols_as_input
                [dims_ext_one{1} dims_ext_one{2}] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Inport, 2);
                size_one = dims_ext_one{1} * dims_ext_one{2};
                [dims_ext_two{1} dims_ext_two{2}] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Inport, 3);
