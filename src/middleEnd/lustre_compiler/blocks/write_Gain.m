...
%% Code
%
function [output_string, var_out] = write_Gain(block, ir_struct, varargin)

var_out = {};

K = LusUtils.getParamValue(ir_struct, block, block.Gain);
multiplication = block.Multiplication;

output_string = '';

[list_out] = list_var_sortie(block);
[list_const] = Utils.list_cst(K, block.CompiledPortDataTypes.Outport(1));
...
[out_dim out_dims] = Utils.get_port_dims(block.CompiledPortDimensions.Outport, 1);
...
if block.CompiledPortComplexSignals.Outport(1)
	...
	[list_const_r list_const_i] = LusUtils.transform_list_const_to_complex(list_const, dt);
end
