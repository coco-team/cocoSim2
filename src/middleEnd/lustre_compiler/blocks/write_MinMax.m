...
%% Code
%
function [output_string, var_out] = write_MinMax(block, ir_struct, varargin)
...
if block.Ports(1) == 1
	...
	if in_dim_r == 1 && in_dim_c == 1
		...
		if numel(list_in) == 2
			...
			for idx_in=1:(numel(list_in)-2)
				...
				varargin{1}.add_Variable(var_name, block.Origin_path, 1, idx_in, true);
	...
	if block.Ports(1) == 2
		...
		for idx_out=1:numel(list_out)
			...
			if_cond = [pre_var_name op list_in{idx_out + (block.Ports(1) - 1) * dim}];
			then_branch = pre_var_name;
			else_branch = [list_in{idx_out + (block.Ports(1) - 1) * dim}];
