...
%% Code
%
function [output_string, var_out] = write_LookupNDDirect(block, ir_struct, varargin)

var_out = {};

nb_dim = LusUtils.getParamValue(ir_struct, block, block.NumberOfTableDimensions);
select = block.InputsSelectThisObjectFromTable;
is_input = block.TableIsInput;
table = LusUtils.getParamValue(ir_struct, block, block.Table);
output_string = '';
...
if selection_size < nb_dim
    inputs_size = block.CompiledPortWidths.Inport(1);
else
    inputs_size = 1;
end
...
if strcmp(is_input, 'off')
    [list_table] = Utils.list_cst(table, block.CompiledPortDataTypes.Outport(1));
    ...
    if block.CompiledPortComplexSignals.Outport(1)
        ...
        [list_table_r, list_table_i] = LusUtils.transform_list_const_to_complex(list_table, dt);
        complex_table = true;
    end
else
    table_as_input = true;
    ...
    for idx_in=1:block.Ports(1)-1
        inputs_indices_size = inputs_indices_size + block.CompiledPortWidths.Inport(idx_in);
    end
    list_table = list_in(inputs_indices_size+1:end);
    [table_dim_r, table_dim_c] = Utils.get_port_dims_simple(block.CompiledPortDimensions.Inport, block.Ports(1));
end
