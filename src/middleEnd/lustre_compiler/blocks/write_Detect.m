...
%% Code
%
function [output_string, var_out] = write_Detect(block, ir_struct, varargin)

output_string = '';
var_out = {};

vinit = LusUtils.getParamValue(ir_struct, block, block.vinit);

[list_out] = list_var_sortie(block);
[list_vinit] = Utils.list_cst(vinit, block.CompiledPortDataTypes.Inport(1));
...
mask = block.MaskType;
if strcmp(mask, BlockUtils.detect_change)
...
elseif strcmp(mask, BlockUtils.detect_dec)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
...
elseif strcmp(mask, BlockUtils.detect_inc)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
...
elseif strcmp(mask, BlockUtils.detect_rise_pos)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
            output_string = app_sprintf(output_string, '\t%s = %s and not (%s -> pre (%s));\n', list_out{idx}, list_in{idx}, list_vinit{idx}, list_in{idx});
        end
...
elseif strcmp(mask, BlockUtils.detect_rise_nonneg)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
...
elseif strcmp(mask, BlockUtils.detect_fall_neg)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
...
elseif strcmp(mask, BlockUtils.detect_fall_nonpos)
    if strcmp(block.CompiledPortDataTypes.Inport(1), 'boolean')
        for idx=1:numel(list_out)
    ...
    error_msg = ['Unhandled masked block: ' block.Origin_path];
    error_msg = [error_msg '\nMask type: ' mask];
