...
%% Code
%
function [output_string, var_out] = write_Concatenate(block, ir_struct, varargin)

var_out = {};

mode = block.Mode;
dim = block.ConcatenateDimension;
...
if block.CompiledPortComplexSignals.Outport(1)
    ...
    for idx_in=1:block.Ports(1)
        ...
        if ~block.CompiledPortComplexSignals.Inport(idx_in)
            for idx=1:nb
                list_in{cpt_in + idx} = LusUtils.real_to_complex_str(list_in{cpt_in + idx}, dt);
            end
        end
        cpt_in = cpt_in + nb;
    end
