function res_assertions = convert_cocospec(obs_inter_blk, list_in, list_in_outport, xml_trace, myblk)
...
for idx_block=1:numel(fields)
    ablock = get_struct(myblk, obs_inter_blk.Content.(fields{idx_block}).Path);
...
if isfield(obs_inter_blk.Content, 'Annotation')
    annots = obs_inter_blk.Content.Annotation;
    
    for idx_annot=1:numel(annots)
        value = obs_inter_blk.Content.(['Annotation' num2str(idx_annot)]).Text;
