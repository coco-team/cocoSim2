...
%% Code
%
function [output_string, c_main_file] = write_extern_SFunction(block, ir_struct)

output_string = '';

function_name = block.FunctionName;
% get port connectivity
props = block.PortConnectivity;
...
for k=1:n_blocks
    ...
    parameters{k,1}=f;
    parameters{k,2}=s.BlockType;
    parameters{k,3}=s.Name;
end
...
try
    ...
    for idx_param=1:nb_param
        param_dt = LusUtils.get_lustre_dt(block.parameters_dt{idx_param});
...
dummy_expr = stub_lustrec(block, list_out, list_in, parameters_string);

varargout = write_entree_sorties_extern_s_function(block, list_out, list_in, parameters_string);
varargout = app_sprintf(varargout, 'let\n');
varargout = app_sprintf(varargout, '\t--! c_code: %s;\n', lustre_annot_name);
varargout = app_sprintf(varargout, ' %s\n', dummy_expr);
varargout = app_sprintf(varargout, 'tel\n\n');
...
if ~isempty(list_in)
    ...
    for idx_input=1:unbloc.Ports(1)
        for idx_dim_in=1:unbloc.CompiledPortWidths.Inport(idx_input)
            in_dt = LusUtils.get_lustre_dt(unbloc.CompiledPortDataTypes.Inport(idx_input));
...
for idx_output=1:unbloc.Ports(2)
    for idx_dim_out=1:unbloc.CompiledPortWidths.Outport(idx_output)
        out_dt = LusUtils.get_lustre_dt(unbloc.CompiledPortDataTypes.Outport(idx_output));
