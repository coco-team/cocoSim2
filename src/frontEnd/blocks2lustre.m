...
% Current subsystem
inter_blk = get_struct(myblk, main_blks{idx_subsys});
fields = fieldnames(inter_blk.Content);
blks = {};
for i=1:numel(fields)
    blks = [blks, inter_blk.Content.(fields{i}).Path];
end
if idx_subsys == 1
    inter_blk = inter_blk.Content.(fields{1});
    blk = inter_blk.Path;
else
    blks = [main_blks{idx_subsys}, blks];
...
if idx_subsys == 1    
	...
	xml_trace.create_Node_Element(inter_blk.Origin_path, node_name);
end;
...
for idx_block=1:nblk
	...
	elseif strcmp(sub_blk.BlockType, 'TriggerPort')
		if strcmp(sub_blk.TriggerType, 'either') && strcmp(sub_blk.ShowOutputPort, 'on')
			...
			str_in_trigg = '';
			str_in_trigg_pre = '';
			out_dt = Utils.get_lustre_dt(inter_blk.CompiledPortDataTypes.Trigger);
			[dim_r dim_c] = Utils.get_port_dims_simple(inter_blk.CompiledPortDimensions.Trigger, 1);
	...
	elseif strcmp(sub_blk.BlockType, 'EnablePort')
		if strcmp(sub_blk.ShowOutputPort, 'on')
			...
			str_in_enable = '';
			out_dt = Utils.get_lustre_dt(inter_blk.CompiledPortDataTypes.Enable);
			[dim_r dim_c] = Utils.get_port_dims_simple(inter_blk.CompiledPortDimensions.Enable, 1);
			for idx_dim=1:(dim_r*dim_c)
...
[is_foriter iter_var_name dt] = Utils.needs_for_iter_var(myblk, inter_blk);
if is_foriter
...
for idx_block=1:nblk
	...
	if strcmp(sub_blk.BlockType, 'Outport')
		...
		outport_dt = Utils.get_lustre_dt(sub_blk.CompiledPortDataTypes.Inport{1});
...
for idx_block=newinit:nblk
	...
	if noutput ~= 0 && ~strcmp(sub_blk.BlockType, 'Inport')
		if ~(strcmp(sub_blk.BlockType, 'SubSystem') && Constants.is_property(sub_blk.MaskType))
			if cpt_var == 1
...
[let_tel_code_string, extern_s_functions_string, extern_functions, properties_nodes, additional_variables, property_node_names, extern_matlab_functions, c_code, external_math_functions] = ...
    write_code(nblk, inter_blk, blks, main_blks, myblk, nom_lustre_file, false, trace, xml_trace);
